...
const onSubmit = async (formEl: FormInstance | undefined) => { // 提交表单
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      ...
      let isPosted = false
      // TODO 加入额外选项
      POST('/usr/reg', { name: regRuleForm.name, email: regRuleForm.email, password: regRuleForm.passwd, organization: regRuleForm.unit, attr: `{ "grade": "${stuGrade.value}" }`, group: regRuleForm.identity, vCode: regRuleForm.emailCaptcha }, (response) => {
        if (response.status === 200 && response.data.statusMsg === 'Success.') {
          console.log(response.data.statusMsg)
          ElMessage({
            message: '注册成功',
